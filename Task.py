...
def create_task_card(parent, index, task):
    card_bg = COLORS["hover"] if index == selected_task_index else COLORS["card"]
    if is_overdue(task):
        title_color = COLORS["danger"]
    else:
        title_color = COLORS["text"]

    card = tk.Frame(parent, bg=card_bg, bd=0, highlightthickness=1, highlightbackground=COLORS["hover"])
    ...
    def on_enter(_):
        if index != selected_task_index:
            card.configure(bg=COLORS["hover"])
            title.configure(bg=COLORS["hover"])
            meta.configure(bg=COLORS["hover"])
            check_btn.configure(bg=COLORS["hover"])
...
def toggle_theme():
    messagebox.showinfo("Тема", "Включена тёплая коричневая тема.")
...
def update_theme_widgets():
    ...
    sidebar.configure(bg=COLORS["sidebar"])
    ...
    bottom_panel.configure(bg=COLORS["sidebar"])
    title_label.configure(bg=COLORS["sidebar"], fg=COLORS["text"])
    ...
    for lbl in form_labels:
        lbl.configure(bg=COLORS["bg"], fg=COLORS["text"], font=("Segoe UI", 11))
    ...
    style_option_menu(priority_menu)
    stats_label.configure(bg=COLORS["sidebar"], fg=COLORS["text"], font=("Segoe UI", 11, "bold"))
    cards_canvas.configure(bg=COLORS["bg"], highlightthickness=0)
    update_filter_buttons()


def style_option_menu(menu_widget):
...
def attach_hover(widget, base_color, hover_color):
    def on_enter(_):
...
def update_filter_buttons():
    ...
    for mode, button in button_mapping.items():
...
for i, btn in enumerate(sidebar_buttons, start=1):
    ...
    attach_hover(btn, COLORS["accent"], COLORS["hover"])
...
create_placeholder(deadline_entry, "дд-мм-гггг")
